...
#         cursor = conn.cursor()
#
#         # Preprocess and chunk the DataFrame
#         rows = []
...
#                 "DOCUMENTTITLE": str(row["DOCUMENTTITLE"]) if pd.notna(row["DOCUMENTTITLE"]) else None,
#                 "PUBLISHED": pd.to_datetime(row["PUBLISHED"]).to_pydatetime() if pd.notna(row["PUBLISHED"]) else None,
#                 "UPDATED": pd.to_datetime(row["UPDATED"]).to_pydatetime() if pd.notna(row["UPDATED"]) else None,
#                 "FILETYPE": str(row["FILETYPE"]) if pd.notna(row["FILETYPE"]) else None,
#                 "FILESIZE": float(row["FILESIZE"]) if pd.notna(row["FILESIZE"]) else None,
#                 "SOURCEURI": str(row["SOURCEURI"]) if pd.notna(row["SOURCEURI"]) else None,
#                 "REPORTINGFREQUENCY": str(row["REPORTINGFREQUENCY"]) if pd.notna(row["REPORTINGFREQUENCY"]) else None,
...
#             values = [[row[col] for col in columns] for row in chunk]
#             execute_values(cursor, insert_query, values)
#             print(f"✅ Inserted/updated chunk {i//chunk_size + 1}")
#
#         conn.commit()
#         cursor.close()
#         conn.close()
...
#         all_rows = []
#         for _, row in joined_df.iterrows():
#             # Build row as dict for DB insert
#             row_dict = {
#                 col: (
#                     pd.to_datetime(row[col]).to_pydatetime()
#                     if col in ["PUBLISHED", "UPDATED"] and pd.notna(row[col])
#                     else row[col] if col != "FILESIZE"
#                     else float(row[col]) if pd.notna(row[col])
#                     else None
#                 )
#                 for col in columns
#             }
#             row_dict["DOCUMENTID"] = str(row["DOCUMENTID"])
#             row_dict["FILESIZE"] = float(row["FILESIZE"]) if pd.notna(row["FILESIZE"]) else None
#             row_dict["OTHERDOCUMENTMETADATA"] = (
#                 row["OTHERDOCUMENTMETADATA"] if isinstance(row["OTHERDOCUMENTMETADATA"], dict) else None
#             )
#             row_dict["ORIGINAL_DOCUMENT"] = str(row["ORIGINAL DOCUMENT"]) if pd.notna(row["ORIGINAL DOCUMENT"]) else None
#
#
#             all_rows.append((row_dict, row))  # also keep full row for TXT
#
#         insert_query = f"""
#             INSERT INTO documents ({", ".join(columns)}) VALUES %s
#             ON CONFLICT (DOCUMENTID) DO UPDATE SET
...
#         cursor = conn.cursor()
#
#         # DB column names (keep underscore version for DB)
#         columns = [
#             "DOCUMENTID", "NAME", "DOCUMENTTITLE", "PUBLISHED", "UPDATED", "FILETYPE",
#             "FILESIZE", "SOURCEURI", "REPORTINGFREQUENCY", "PRIMARYENTITYTYPE",
#             "PRIMARYENTITYNAME", "DOCUMENT_PRIMARY_ENTITY_ID", "OTHERDOCUMENTMETADATA",
...
#
#         # Filter DataFrame: keep new or updated rows only
#         filtered_df = joined_df[
#             (joined_df["DOCUMENTID"].astype(str).apply(lambda x: x not in existing_records)) |
#             (
#                 joined_df["DOCUMENTID"].astype(str).apply(lambda x: x in existing_records) &
#                 joined_df["UPDATED"].apply(pd.to_datetime) >
#                 joined_df["DOCUMENTID"].astype(str).apply(lambda x: existing_records.get(x))
#             )
#         ]
#
#         # DB column names
